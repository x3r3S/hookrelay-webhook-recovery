...
`tests/domain.test.mjs` covers the individual decisions. `tests/proof.test.mjs` adds one end-to-end fixture batch and checks the committed audit file against a fresh in-memory run. If the implementation changes without updating the evidence, that comparison fails.

The static page has no bundler or runtime dependency. All browser imports and asset links are relative, which also keeps it suitable for a repository-root GitHub Pages deployment.
