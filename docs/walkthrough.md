...
3. Select `invoice.finalized`. Four failed attempts place it in the dead-letter view. Use **Replay event** once and confirm that the local state changes to `delivered` and the latest audit shows `operator replay · 200 · local simulation`.
...
6. Download the audit JSON. It should still report four ingress decisions—two accepted, one duplicate and one rejected—plus one separate `operatorAudit` entry with `boundary: "browser_local_simulation"` and `externalAction: false`.
